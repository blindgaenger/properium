...
function validateValue(propPath, value, prop) {
  const required = prop.required
  if (_.isUndefined(value)) { // TODO: _.isNull, _.isEmpty
    if (required) ProperiumError.fail(propPath, 'required value')
...
function validateProp(propPath, object, prop) {
  const propName = prop.name

  const defaultValue = prop.defaultValue
  if (!_.has(object, propName)) {
    if (!defaultValue) ProperiumError.fail([propPath, propName], 'undefined prop')
    _.set(object, propName, defaultValue)
  }

  const value = _.get(object, propName)
  validateValue([propPath, propName], value, prop)
