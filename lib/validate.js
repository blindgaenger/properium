...
function _assertAllKnown(propPath, object, props) {
  const knownPropNames = _.map(props, 'name')
  _.keys(object).forEach(objectPropName => {
    if (!_.includes(knownPropNames, objectPropName)) ProperiumError.fail([propPath, objectPropName], 'unknown prop', `found a prop, that was not defined: ${objectPropName}`)
  })
}

function _assertDefined(propPath, object, prop) {
  const name = prop.name
  if (!_.has(object, name)) ProperiumError.fail(propPath, 'undefined prop', 'must explicitly set prop, even to undefined')
}
...
function _assertRequired(propPath, value, prop) {
  ...
  if (required) {
    if (_.isUndefined(value)) ProperiumError.fail(propPath, 'required value', `expected value, but was ${value}`)
  }
}
...
function validateProps(rootPath, object, props) {
  _assertAllKnown(rootPath, object, props)
  props.forEach(prop => validateProp(rootPath, object, prop))
  return object
}
...
function validateProp(rootPath, object, prop) {
  ...
  if (value && value.validate) value.validate(propPath)
